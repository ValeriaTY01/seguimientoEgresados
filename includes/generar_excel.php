...
// Determinar el rol
$rol = $_SESSION['rol'] ?? '';
$isAdmin = $rol === 'administrador' || $rol === 'jefe vinculación';
$isJefe = $rol === 'jefe departamento';

// Determinar carrera según el rol
$carrera = '';
if ($isJefe) {
...
// Obtener secciones según tipo de encuesta (química o general)
if ($carrera === 'Ingeniería Química' || $carrera === 'Ingeniería Bioquímica') {
...
while ($seccion = $secciones->fetch_assoc()) {
    ...
    $stmtRespuestas->bind_param("s", $carrera);
...
$stmtEgresados->bind_param("s", $carrera);
...
// Hoja Empresas (solo si no es carrera química)
if ($carrera !== 'Ingeniería Química' && $carrera !== 'Ingeniería Bioquímica') {
    ...
    $stmtEmpresas->bind_param("s", $carrera);
